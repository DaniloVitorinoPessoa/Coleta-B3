...
class DatabaseManager:
        ...
        try:
            ...
            with self.engine.connect() as conn:
                ...
                try:
                    ...
                    conn.execute(
                        text(
                            "ALTER TABLE ativos ADD COLUMN IF NOT EXISTS setor VARCHAR(80)"
                        )
                    )
                    conn.commit()
                ...
                try:
                    ...
                    if result.fetchone():
                        conn.execute(
                            text("ALTER TABLE cotacoes DROP COLUMN quantidade")
                        )
                        conn.commit()
                        logger.info("Coluna 'quantidade' removida da tabela cotacoes")
                except Exception as e:
                    logger.warning(
                        f"Erro ao remover coluna quantidade (pode não existir): {e}"
                    )
                ...
                # 4. Remover ativos que não estão mais na coleta atual
                ...
                codes_to_remove = [
                    code for code in current_codes_db if code not in new_codes
                ]
                ...
                if codes_to_remove:
                    for code in codes_to_remove:
                        ...
                        if not has_data:
                            ...
                            conn.execute(
                                text("DELETE FROM ativos WHERE codigo = :codigo"),
                                {"codigo": code},
                            )
                            removed_count += 1
    ...
    def insert_cotacoes(self, df_cotacoes, data_referencia):
        ...
        try:
            ...
            with self.engine.connect() as conn:
                ...
                result = conn.execute(
                    text("DELETE FROM cotacoes WHERE DATE(data) = :target_date"),
                ...
                logger.info(
                    f"Removidas {deleted_count} cotacoes existentes para {data_str}"
                )

                # Verificar se ainda existem duplicatas
                check_result = conn.execute(
                    text(
                        "SELECT COUNT(*) as count FROM cotacoes WHERE DATE(data) = :target_date"
                    ),
                    {"target_date": data_str},
                )
                ...
                if remaining > 0:
                    logger.warning(
                        f"Ainda existem {remaining} cotacoes para {data_str}. Removendo novamente..."
                    )
                    conn.execute(
                        text("DELETE FROM cotacoes WHERE DATE(data) = :target_date"),
                        {"target_date": data_str},
            ...
            with self.engine.connect() as conn:
                for i in range(0, len(df_cotacoes), batch_size):
                        ...
                        insert_data.append(
                            {
                                ...
                                "data": row["data"],
                                "preco_abertura": (
                                    float(row["preco_abertura"])
                                    if pd.notna(row["preco_abertura"])
                                    else None
                                ),
                                "preco_fechamento": (
                                    float(row["preco_fechamento"])
                                    if pd.notna(row["preco_fechamento"])
                                    else None
                                ),
                                "maximo": (
                                    float(row["maximo"])
                                    if pd.notna(row["maximo"])
                                    else None
                                ),
                                "minimo": (
                                    float(row["minimo"])
                                    if pd.notna(row["minimo"])
                                    else None
                                ),
                                "negocios": (
                                    int(row["negocios"])
                                    if pd.notna(row["negocios"])
                                    else None
                                ),
                                "volume_financeiro": (
                                    float(row["volume_financeiro"])
                                    if pd.notna(row["volume_financeiro"])
                                    else None
                                ),
                            }
                        )
                    ...
                    logger.info(
                        f"Inserido lote {i//batch_size + 1}: {len(insert_data)} cotacoes"
                    )
    ...
    def execute_query(self, query, params=None):
        ...
        try:
            if params:
                # Para pandas, os parâmetros devem ser passados como tupla
                # E converter tipos numpy para tipos Python nativos
                processed_params = []
                for param in params if isinstance(params, (list, tuple)) else [params]:
                    # Converter numpy types para Python types
                    if hasattr(param, "item"):  # numpy types têm método .item()
                        processed_params.append(param.item())
                    else:
    ...
    def insert_dividendos(self, df_dividendos, ativos_db):
        ...
        try:
            ...
            df_merged = df_merged.dropna(
                subset=["id"]
            )  # Remover ativos não encontrados
    ...
    def check_and_create_tables(self):
        ...
        try:
            ...
            """
            existing_tables = pd.read_sql(tables_query, self.engine)[
                "table_name"
            ].tolist()
            ...
            if missing_tables:
                ...
                with self.engine.connect() as conn:
                    for table in missing_tables:
                        # Para tabelas faltando, usar o schema.sql completo
                        logger.info(f"Tabela {table} deve ser criada via schema.sql")
                    ...
                    if os.path.exists(schema_path):
                        ...
                        logger.info("Schema completo executado")
                        # Sair do loop após executar o schema completo
                        # (a execução do schema já cria todas as tabelas necessárias)
                        conn.commit()
