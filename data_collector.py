...
class B3DataCollector:
    ...
    def download_cotahist(self):
        ...
        try:
            ...
            # Fazer uma requisição HEAD primeiro para verificar se o arquivo existe
            try:
                head_response = requests.head(url, headers=headers, timeout=10)
                logger.info(f"Status da verificação HEAD: {head_response.status_code}")

                if head_response.status_code == 404:
                    # Tentar ano anterior se o arquivo do ano atual não existir
                    ano_anterior = datetime.now().year - 1
                    url_anterior = f"https://bvmf.bmfbovespa.com.br/InstDados/SerHist/COTAHIST_A{ano_anterior}.ZIP"
                    logger.info(
                        f"Arquivo do ano atual não encontrado, tentando ano anterior: {url_anterior}"
                    )
                    url = url_anterior

            except requests.exceptions.RequestException as e:
                logger.warning(
                    f"Erro na verificação HEAD: {e}. Continuando com download direto."
                )

            response = requests.get(
                url, headers=headers, timeout=self.config.get("timeout", 30)
            )
            ...
            if not response.content.startswith(b"PK"):
                logger.error("Arquivo baixado não é um ZIP válido")
                logger.info(f"Primeiros 100 bytes: {response.content[:100]}")
                return None
    ...
    def parse_csv_data(self, zip_file):
        ...
        try:
            ...
            for encoding in encodings:
                try:
                    # PRIMEIRO: tentar com posições fixas (formato COTAHIST tradicional)
                    logger.info(
                        f"Tentando leitura como posições fixas com encoding {encoding}"
                    )
                    df = pd.read_fwf(
                    ...
                    if not df.empty:
                        logger.info(
                            f"CSV lido como posições fixas com encoding {encoding}. Linhas: {len(df)}"
                        )
                        break

                except Exception as e:
                    logger.warning(
                        f"Falha como posições fixas com encoding {encoding}: {e}"
                    )

                try:
                    # FALLBACK: tentar como CSV delimitado por ponto e vírgula
                    logger.info(
                        f"Tentando leitura como CSV delimitado com encoding {encoding}"
                    )
                    df = pd.read_csv(
                        zip_file.open(zip_file.namelist()[0]),
                        sep=";",
                    ...
                    if not df.empty and len(df.columns) >= 10:
                        logger.info(
                            f"CSV lido como delimitado com encoding {encoding}. Linhas: {len(df)}"
                        )
                        break

                except Exception as e:
                    logger.warning(
                        f"Falha como CSV delimitado com encoding {encoding}: {e}"
                    )
                    continue
            ...
            logger.info(f"Primeiras 3 linhas do CSV:")
            for i, row in df.head(3).iterrows():
                logger.info(
                    f"Linha {i}: TIPREG={row.get('TIPREG', 'N/A')}, DATA={row.get('DATA', 'N/A')}, CODNEG={row.get('CODNEG', 'N/A')}"
                )
    ...
    def filter_d1_data(self, df):
        ...
        try:
            ...
            logger.info(
                f"Apos filtro D-1: {len(df_filtered)} registros para {self.data_d1}"
            )
            ...
            if len(df_filtered) == 0:
                ...
                logger.info(
                    f"Datas disponíveis no arquivo: {datas_ordenadas[-5:]}"
                )  # Últimas 5 datas
                ...
                if len(datas_ordenadas) > 0:
                    ...
                    logger.info(
                        f"Usando data mais recente: {data_mais_recente} ({len(df_filtered)} registros)"
                    )
        ...
        try:
            ...
            for col in price_columns:
                if col in df_transformed.columns:
                    df_transformed[col] = (
                        pd.to_numeric(df_transformed[col], errors="coerce") / 100
                    )

            # Converter volumes e quantidades
            if "volume_financeiro" in df_transformed.columns:
                df_transformed["volume_financeiro"] = (
                    pd.to_numeric(df_transformed["volume_financeiro"], errors="coerce")
                    / 100
                )

            if "negocios" in df_transformed.columns:
                df_transformed["negocios"] = pd.to_numeric(
                    df_transformed["negocios"], errors="coerce"
                )
            ...
            logger.info(
                f"Dados transformados com sucesso. {len(df_transformed)} registros válidos"
            )
        ...
        try:
            ...
            if codigo.endswith("11"):
                ...
                if any(word in nome_upper for word in ["SHOPPING", "MALL", "VAREJO"]):
                    ...
                    word in nome_upper
                    for word in ["LOGISTICO", "LOGÍSTICA", "GALPAO", "GALPÃO"]
                ):
                    setor = "Logística"
                elif any(
                    word in nome_upper
                    for word in ["CORPORATIVO", "LAJES", "ESCRITORIO", "ESCRITÓRIO"]
                ):
                    setor = "Corporativo"
                elif any(
                    word in nome_upper for word in ["RESIDENCIAL", "HABITACIONAL"]
                ):
                    setor = "Residencial"
                elif any(
                    word in nome_upper
                    for word in ["HOSPITAL", "SAUDE", "SAÚDE", "CLINICA", "CLÍNICA"]
                ):
                    setor = "Saúde"
                elif any(word in nome_upper for word in ["HOTEL", "HOTELARIA"]):
                    setor = "Hotelaria"
                elif any(
                    word in nome_upper for word in ["AGRO", "AGRICOLA", "AGRÍCOLA"]
                ):
                    setor = "Agronegócio"
                elif any(
                    word in nome_upper
                    for word in ["PAPEL", "CRI", "CREDITO", "CRÉDITO"]
                ):
                    setor = "Papel e Renda"
                else:
                    setor = "Outros"
                ...
                tipo = "BDR"
                setor = "Internacional"
            elif "ETF" in nome.upper() or any(
                codigo.startswith(prefix) for prefix in ["BOVA", "SMAL", "IVVB"]
            ):
                tipo = "ETF"
                setor = "Índices"
            else:
                ...
                if any(
                    word in nome_upper
                    for word in ["PETRO", "OLEO", "GAS", "COMBUSTIVEL"]
                ):
                    setor = "Petróleo e Gás"
                elif any(
                    word in nome_upper
                    for word in ["BANCO", "BRADESCO", "ITAU", "SANTANDER", "FINANC"]
                ):
                    setor = "Bancos"
                elif any(
                    word in nome_upper
                    for word in ["VALE", "MINERA", "SIDERUR", "METAL", "ACO", "AÇO"]
                ):
                    setor = "Mineração e Siderurgia"
                elif any(
                    word in nome_upper
                    for word in ["ELETRIC", "ENERGIA", "ENERG", "CEMIG", "COPEL"]
                ):
                    setor = "Energia Elétrica"
                elif any(
                    word in nome_upper
                    for word in ["TELEFON", "TELECOM", "TIM", "VIVO", "OI"]
                ):
                    setor = "Telecomunicações"
                elif any(
                    word in nome_upper
                    for word in ["CONSTRUC", "CIVIL", "MRV", "CYRELA", "GAFISA"]
                ):
                    setor = "Construção Civil"
                elif any(
                    word in nome_upper
                    for word in ["VAREJO", "MAGALU", "VIA", "AMERICANAS", "LOJAS"]
                ):
                    setor = "Varejo"
                elif any(
                    word in nome_upper
                    for word in ["ALIMENT", "BEBIDA", "BRF", "AMBEV", "JBS"]
                ):
                    setor = "Alimentos e Bebidas"
                elif any(
                    word in nome_upper
                    for word in ["SAUDE", "HOSPITAL", "MEDIC", "QUALICORP"]
                ):
                    setor = "Saúde"
                elif any(
                    word in nome_upper
                    for word in ["PAPEL", "CELULOSE", "SUZANO", "KLABIN"]
                ):
                    setor = "Papel e Celulose"
                elif any(
                    word in nome_upper for word in ["SEGUR", "PORTO", "SUL AMERICA"]
                ):
                    setor = "Seguros"
                elif any(
                    word in nome_upper
                    for word in ["TRANSPORT", "LOGISTIC", "RUMO", "AZUL"]
                ):
                    setor = "Transporte"
                else:
                    setor = "Outros"
    ...
    def collect_historical_data(self, dias_historico=30):
        """
        Coleta dados históricos simulados para os últimos N dias

        Args:
            dias_historico (int): Número de dias históricos para simular

        Returns:
            tuple: (df_cotacoes_historicas, df_ativos, df_dividendos)
        """
        try:
            logger.info(
                f"Iniciando coleta de dados históricos para {dias_historico} dias"
            )

            # Primeiro, coletar dados atuais
            df_atual, df_ativos, df_dividendos = self.collect_and_process_data()

            if df_atual is None or df_atual.empty:
                logger.warning("Não foi possível coletar dados atuais")
                return None, None, None

            # Verificar se df_atual tem a estrutura correta
            logger.info(f"Colunas do df_atual: {df_atual.columns.tolist()}")

            # Criar dados históricos simulados baseados nos dados atuais
            df_historico_completo = []

            for i in range(dias_historico):
                data_simulada = datetime.now().date() - timedelta(days=i)

                # Pular fins de semana (sábado=5, domingo=6)
                if data_simulada.weekday() >= 5:
                    continue

                logger.info(f"Gerando dados simulados para {data_simulada}")

                # Copiar dados atuais e modificar data e preços
                df_dia = df_atual.copy()
                df_dia["data"] = data_simulada

                # Simular variação de preços (±5% aleatório)
                import random

                for idx in df_dia.index:
                    variacao = random.uniform(-0.05, 0.05)  # ±5%

                    preco_base = df_dia.loc[idx, "preco_fechamento"]
                    nova_variacao = preco_base * variacao

                    # Aplicar variação mantendo lógica OHLC
                    df_dia.loc[idx, "preco_abertura"] = preco_base + (
                        nova_variacao * 0.8
                    )
                    df_dia.loc[idx, "preco_fechamento"] = preco_base + nova_variacao
                    df_dia.loc[idx, "maximo"] = (
                        max(
                            df_dia.loc[idx, "preco_abertura"],
                            df_dia.loc[idx, "preco_fechamento"],
                        )
                        * 1.02
                    )
                    df_dia.loc[idx, "minimo"] = (
                        min(
                            df_dia.loc[idx, "preco_abertura"],
                            df_dia.loc[idx, "preco_fechamento"],
                        )
                    ...
                    # Simular volume (±30%)
                    if pd.notna(df_dia.loc[idx, "volume_financeiro"]):
                        variacao_volume = random.uniform(0.7, 1.3)
                        df_dia.loc[idx, "volume_financeiro"] *= variacao_volume

                df_historico_completo.append(df_dia)

            # Combinar todos os dias
            if df_historico_completo:
                df_final = pd.concat(df_historico_completo, ignore_index=True)
                df_final = df_final.sort_values("data").reset_index(drop=True)

                logger.info(
                    f"Dados históricos gerados: {len(df_final)} registros para {len(df_historico_completo)} dias"
                )
                logger.info(f"Colunas do df_final: {df_final.columns.tolist()}")

                return df_final, df_ativos, df_dividendos
            else:
                logger.warning("Nenhum dado histórico foi gerado")
                return df_atual, df_ativos, df_dividendos
